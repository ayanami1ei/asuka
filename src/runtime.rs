...
// ── IrNode trait (generated by derive) ──
...
/// Trait for user-defined IR node behaviour (lower, emit, etc.)
///
/// Each node type implements these methods directly.
/// Child nodes are accessed via self fields and called via trait object dispatch.
///
/// # Example
/// ```
/// use asuka::runtime::IrNodeKind;
///
...
pub trait IrNode: IrNodeKind {
    /// Lower this node to the next IR level.
    fn lower(&self) -> String { String::new() }
    /// Emit target code for this node.
    fn emit(&self) -> String { String::new() }
}
